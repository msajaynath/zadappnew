...
export class RecipieService {
  ...
  CreatedRecipe:any;
	...
	GetAllRecipies(Categories: Array<string>, Cusines: Array<string>, CookingType : Array<string>, time: string, calories: string){
		...
		this.Recipies = [];
		let search;
		        if(Categories.length==0&&Cusines.length==0&&CookingType.length==0)
				{
				search='';	
				}
       else
    ...
    alert(search);
	return new Promise(resolve=>{  this.http.get(this.Url + 'getAllRecipes?search='+search)
        .subscribe(data => {
			debugger;
          ///this.CreatedRecipe=data["_body"];
            resolve(data.json());
	...
	GetAllRecipiesByUserId(UserId: string){
		this.Recipies = [];
			...
			// 	Time : "25",
			// 	Images: [{URL: "assets/Images/PanCakes.jpg",Order:0,Type:""}] ,
			// 	Liked: false,
			// 	MakerId: "dfh54dfhdf4",
			...
			// 	Time : "15",
			// 	Images: [{s3Url: 'assets/Images/Chat.jpg', isPrimary: true}] ,
			// 	Liked: true,
			// 	MakerId: "dfh54dfhdf4",
